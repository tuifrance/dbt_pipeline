...
with
    ...
    consolidation as (
        select
            ...
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 41
            ) as destination,
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 33
            ) as ville_depart,
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 81
            ) as ville_arrive,
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 70
            ) as type_produit,
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 22
            ) as date_depart,
            (
                select x.value from unnest(h.customdimensions) as x where x.index = 24
            ) as date_arrive,
            (
