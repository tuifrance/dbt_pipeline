    ...
    consolidation as (
        select
            ...
            count(
                distinct case
                    when
                        h.eventinfo.eventcategory = 'Recherche Destination - Moteur Vol'
                    then concat(fullvisitorid, cast(visitstarttime as string))
                end
            ) as recherche_vols,
            count(
            ...
            count(
                distinct case
                    when h.eventinfo.eventcategory = 'Création - Compte Client Tunnel '
                    then concat(fullvisitorid, cast(visitstarttime as string))
                end
            ) as creation_compte,
            ...
            sum(
                case
                    when channelgrouping = 'SEO' then h.transaction.transactionrevenue
                end
            ) as seo_revenue,
            sum(
                case
                    when channelgrouping = 'SEA' then h.transaction.transactionrevenue
                end
            ) as sea_revenue,
            sum(
                case
            ...
            sum(
                case
                    when channelgrouping = 'E-CRM' then h.transaction.transactionrevenue
                end
            ) as ecrm_revenue,
            sum(
                case
            ...
            count(
                distinct case
                    when channelgrouping = 'Référents' then h.transaction.transactionid
                end
            ...
            count(
                distinct case
                    when channelgrouping = 'Display' then h.transaction.transactionid
                end
            ...
            count(
                distinct case
                    when channelgrouping = '(Other)' then h.transaction.transactionid
                end
