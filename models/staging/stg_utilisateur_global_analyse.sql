...
count( distinct case when h.eventInfo.eventCategory ='Utilisation Moteur HP'  and h.eventInfo.eventAction='MoteurHPPackage' then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))  end ) as recherche_packages,
count( distinct case when h.eventInfo.eventCategory ='Recherche Destination - Moteur Vol'   then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))  end ) as recherche_vols,
count( distinct case when h.eventInfo.eventCategory ='Option Package' then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))  end ) as  option_packages,
count( distinct case when h.eventInfo.eventCategory ='Connexion - Compte Client Tunnel' then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))  end ) as  Connexion_copte,
count( distinct case when h.eventInfo.eventCategory ='Création - Compte Client Tunnel ' then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING))  end ) as  Creation_compte, 
...
count(distinct case when channelGrouping= 'Google not provided' then CONCAT(fullVisitorId, CAST(visitStartTime AS STRING)) end ) as not_provided_sessions,

sum(case when channelGrouping= 'SEO' then h.transaction.transactionRevenue end ) as seo_revenue,
sum(case when channelGrouping= 'SEA' then h.transaction.transactionRevenue  end ) as sea_revenue,
sum(case when channelGrouping= 'Accès direct' then h.transaction.transactionRevenue  end ) as direct_revenue,
sum(case when channelGrouping= 'Référents' then h.transaction.transactionRevenue  end ) as referent_revenue,
sum(case when channelGrouping= 'Affiliation' then h.transaction.transactionRevenue end ) as affiliation_revenue,
sum(case when channelGrouping= 'E-CRM' then h.transaction.transactionRevenue end ) as ecrm_revenue,
sum(case when channelGrouping= 'Display' then h.transaction.transactionRevenue  end ) as display_revenue,
sum(case when channelGrouping= 'Social' then h.transaction.transactionRevenue  end ) as social_revenue,
...
count(distinct case when channelGrouping= 'Google not provided' then h.transaction.transactionId end ) as not_provided_transaction
