...
select
 ...
 select distinct
   ...
   (SELECT x.value FROM UNNEST(h.customDimensions) as x WHERE x.index = 29) as code_produit,
    ...
    (SELECT x.value FROM UNNEST(h.customDimensions) as x WHERE x.index = 77) as ville_depart,
    ...
    (SELECT x.value FROM UNNEST(h.customDimensions) as x WHERE x.index = 42) as continent,
