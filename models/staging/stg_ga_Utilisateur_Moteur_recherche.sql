...
with
    ...
    consolidation as (
        select
            ...
            (
                select x.value from unnest(h.customdimensions) x where x.index = 41
            ) as destination,
            (
                select x.value from unnest(h.customdimensions) x where x.index = 33
            ) as ville_depart,
            (
                select x.value from unnest(h.customdimensions) x where x.index = 22
            ) as date_depart,
            (
                select x.value from unnest(h.customdimensions) x where x.index = 143
            ) as duree_voyage,
            (
